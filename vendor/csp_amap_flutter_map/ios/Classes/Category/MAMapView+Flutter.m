...
/// 根据 Flutter 侧传入的 cameraUpdate 指令，更新地图状态
- (void)setCameraUpdateDict:(NSDictionary *)updateDict {
    ...
    // cameraUpdate 数组，第一个元素为操作符，后面为参数
    NSAssert(cameraUpdate.count >= 1, @"cameraUpdate 参数错误");
    NSString *operation = cameraUpdate.firstObject;
    if ([operation isEqualToString:@"newCameraPosition"] && cameraUpdate.count == 2) {// 按照 cameraPosition 移动
    ...
    } else if ([operation isEqualToString:@"newLatLngZoom"] && cameraUpdate.count == 3) {// 设置地图中心点和 zoomLevel
        CLLocationCoordinate2D position = [AMapConvertUtil coordinateFromArray:cameraUpdate[1]];
...
/// 根据 Flutter 侧传入的 options 字典，批量设置地图参数
- (void)updateMapViewOption:(NSDictionary *)dict withRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    ...
    // 独立的定位开关（未设置 myLocationStyle 时生效，与 Android/HarmonyOS 逻辑一致）
    NSNumber *myLocationEnabled = dict[@"myLocationEnabled"];
    if (myLocationEnabled && !locationStyleDict) {
        self.showsUserLocation = [myLocationEnabled boolValue];
        if ([myLocationEnabled boolValue]) {
            self.userTrackingMode = MAUserTrackingModeNone;// 强制设置为非追随模式
        }
    }
    ...
    CGSize logoSize = CGSizeMake(110, 40); // logo的默认大小
