...
// 数学/物理常量按惯例使用 UPPER_SNAKE_CASE 命名，此处忽略该 lint
...
class AMapTools {
  ...
  static bool latLngIsInPolygon(LatLng latLng, List<LatLng> latLngList) {
    ...
    bool isInside = false;
    int count = 0;
    final double linePoint1x = latLng.longitude;
    final double linePoint1y = latLng.latitude;
    const double linePoint2x = 180;
    final double linePoint2y = latLng.latitude;

    for (int i = 0; i < polygon.length - 1; i++) {
      final double cx1 = polygon[i].longitude;
      final double cy1 = polygon[i].latitude;
      final double cx2 = polygon[i + 1].longitude;
      final double cy2 = polygon[i + 1].latitude;
