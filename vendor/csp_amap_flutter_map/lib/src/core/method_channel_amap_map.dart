...
import 'package:csp_amap_flutter_map/src/core/amap_flutter_platform.dart';
import 'package:csp_amap_flutter_map/src/types/types.dart';
...
const String VIEW_TYPE = 'csp_amap_flutter_map';
...
class MethodChannelAMapFlutterMap implements AMapFlutterPlatform {
  ...
  Future<void> init(int mapId) {
    ...
    if (channel == null) {
      channel = MethodChannel('csp_amap_flutter_map_$mapId');
  ...
  Widget buildView(
    ...
    // 其他平台使用AndroidView作为后备
    // 注意：HarmonyOS支持请参见项目文档，可在Gitee仓库获取完整版本
    return AndroidView(
      viewType: VIEW_TYPE,
      onPlatformViewCreated: onPlatformViewCreated,
      gestureRecognizers: gestureRecognizers,
  ...
  Stream<CameraMoveEndEvent> onCameraMoveEnd({required int mapId}) {
    return _events(mapId)
        .where((event) => event is CameraMoveEndEvent)
        .cast<CameraMoveEndEvent>();
  }
